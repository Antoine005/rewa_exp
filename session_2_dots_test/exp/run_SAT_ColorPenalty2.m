...
if pretest==1
    ...
    % Open up results file for writing subjects' behavioral results to file.
    resultsfile=['data/Test_' subID '_SAT_Color2.txt'];
    fid=fopen(resultsfile,'a');
end
load(inputfile);
...
if blockNo == 1
    ...
    centerText(screenInfo.curWindow,'After fixation, Red and Green circles appeared with white arc on the screen.',xCtr,yCtr-200,white)
    centerText(screenInfo.curWindow,'The arc represented the available time, the Red and Green dots appeared randomly ',xCtr,yCtr-150,white)
    centerText(screenInfo.curWindow,'around the center of the screen while the size of the arc started to decrease.',xCtr,yCtr-100,white)
    ...
    centerText(screenInfo.curWindow,'If you could not decide your choice, you can press "SPACE" (for SKIP).',xCtr,yCtr,white)
    centerText(screenInfo.curWindow,'You will receive feedback on your performance using points. Once you reach a total score of 100 points, the task is over.',xCtr,yCtr+50,white)
    centerText(screenInfo.curWindow,' Each time you can earn 1 point if you choose the correct answer. You lose 3 points for an incorrect choice.',xCtr,yCtr+100,white)
    centerText(screenInfo.curWindow,'If you miss the deadline you will lose 3 points.',xCtr,yCtr+150,white)
    centerText(screenInfo.curWindow,'So make sure you dont miss the deadline!',xCtr,yCtr+200,white)
    centerText(screenInfo.curWindow,'If you skip the trial, you will not lose any point.',xCtr,yCtr+250,white)
