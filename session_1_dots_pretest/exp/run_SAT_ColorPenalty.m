...
global red yellow gap white inputs screenInfo rightKey leftKey skipKey xCtr yCtr width_fix buttonPress
%% clean the screen
% hide the cursor
HideCursor 
...
% answer setting
rightKey ='right'; % 1 = choose right option
leftKey  ='left'; % 2 = choose left  option
skipKey = ' '; %space to correct
...
%centerText(screenInfo.curWindow,['Block ' num2str(blockNo)],xCtr,yCtr-100,white)
centerText(screenInfo.curWindow,'Welcome to the experiment!',xCtr,yCtr-100,white)
centerText(screenInfo.curWindow,'In the next hour you will be asked to do one task.',xCtr,yCtr-50,white)
centerText(screenInfo.curWindow,'The intention is to choose the dominant color (the most presented color)',xCtr,yCtr,white)
centerText(screenInfo.curWindow,'in a patch of Red and Green dots, within different deadlines.',xCtr,yCtr+50,white)
centerText(screenInfo.curWindow,'You do this by pressing the "right arrow" (for RIGHT circle) and "left arrow" (for LEFT circle).',xCtr,yCtr+100,white)
...
for trialNo=1:nTrialsPB
    ...
    [chooseLeft,RT,skip,buttonPress] = sampling_SAT_color(blockNo,trialNo,max_time);
    ...
    if chooseR == -2
            centerText(screenInfo.curWindow,'Skipped: You get 0 point.',xCtr,yCtr-6,white)
            correct=0;
            win=0;
    elseif chooseR~= -1 && chooseR~=-2
        ...
        if abs(chooseR - correctIsR) ==0
            win=rew_correct;
            correct=1;
            centerText(screenInfo.curWindow,['Correct: Win ' num2str(win) ' points'],xCtr,yCtr-6,yellow)
        elseif abs(chooseR - correctIsR) ==1
            win=-rew_wrong;
            correct=0;
            centerText(screenInfo.curWindow,['Wrong: Loss ' num2str(rew_wrong) ' points'],xCtr,yCtr-6,white)
        end
        
    elseif chooseR==-1
        win=-rew_tooSlow;
        correct=-1;
        centerText(screenInfo.curWindow,['Too Slow! Loss ' num2str(rew_tooSlow) ' points'],xCtr,yCtr-6,white)
