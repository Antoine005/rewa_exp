function [chooseLeft,RT,skip,buttonPress] = sampling_SAT_color(blockNo,trialNo,max_time,version)
%
...
global inputs screenInfo xCtr yCtr white width_fix rightKey leftKey skipKey
...
arc_range      = apD*2+10;
Screen('FrameArc',curWindow, grey, [xCtr-arc_range yCtr-arc_range xCtr+arc_range yCtr+arc_range], 0,360,4);
...
start_time = GetSecs;
        ...
        if frames>=1
            ...
            if buttonPress
                ...
                elseif strcmp(KbName(keyCode),leftKey)
                    ...
                    can_not_change=1;
                    break
                elseif strcmp(KbName(keyCode),skipKey)
                    skip=1;
                    chooseLeft=-2;
                    RT = time-start_time;
                    can_not_change=1;
    ...
    elseif chooseLeft == -2
        Screen('FillOval', curWindow, white, [dir_show(choice(1),1)-radius_choice, dir_show(choice(1),2)-radius_choice, dir_show(choice(1),1)+radius_choice, dir_show(choice(1),2)+radius_choice]');
        Screen('FillOval', curWindow, white, [dir_show(choice(2),1)-radius_choice, dir_show(choice(2),2)-radius_choice, dir_show(choice(2),1)+radius_choice, dir_show(choice(2),2)+radius_choice]');
    end
