function [paramsHat,pC_hat] = getMLE_SAT_Color_lottery(rt_mat,min_RT,subID,funcForm,max_time,session)
%
% 2012.12.28. SWW.
...
hold on;
h = figure(1); hold on
plot(rt,pC_hat,color(session),'linewidth',3);
% axis([-0.04 max_time 0.4 1.1])
axis([-0.04 5 0.4 1.1])
axis square;
hold on;
plot([lb_rt lb_rt],[0.4 1.1],color(session))
plot([tHat_riseFromChance tHat_riseFromChance],[0.4 1.1],'g');
pCorrect = data(:,2)./data(:,3);
n_Cor=data(:,2);
n_Wrong=data(:,3)-data(:,2);

% Color_SHOW=['b','g','r','m','k','c'];
Color_SHOW=['k','k','k','k','k','k' ; 'b','b','b','b','b','b' ];
for nPlot=1:length(data(:,3));
stdCorrect=std([ones(n_Cor(nPlot),1);zeros(n_Wrong(nPlot),1)])/data(nPlot,3)^0.5;
plot([mu_RT(nPlot) mu_RT(nPlot)],[pCorrect(nPlot) pCorrect(nPlot)+stdCorrect],Color_SHOW(session,nPlot),'linewidth',4);
plot([mu_RT(nPlot) mu_RT(nPlot)],[pCorrect(nPlot) pCorrect(nPlot)-stdCorrect],Color_SHOW(session,nPlot),'linewidth',4);
plot(mu_RT(nPlot),pCorrect(nPlot),'o', 'MarkerFaceColor', Color_SHOW(session,nPlot),'markersize',10 );
end
...
saveas(h,subID,'jpg')
% %% correlation
% pC_comapre(:,session) = pC_hat(1:420);
% if session == 2
%     R = corrcoef(pC_comapre)
% end
