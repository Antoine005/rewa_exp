...
t_stim_pass = 1/(inputs(blockNo).sampleRate(1,1)*dTime);
...
% draw the choice   
   ...
   if version ~=4
        %time_total_1 = time_total; The original code that I can't see.
   ...
   % startAngle = sizeAngle;
    ...
    if mod(indx,round(time_total/(currTime*inputs(blockNo).sampleRate(1,1)))) == 0 
       
        x = x+1;
        count = count + 1;
        t_stim_pass = t_stim_pass + (1/(inputs(blockNo).sampleRate(1,1)));
    end
    frames = frames + 1;
